...
# YouTube DL setup
class YDL(discord.PCMVolumeTransformer):
    ydl_opts = {
    ...
    async def create_source(cls, ctx, search, *, loop: asyncio.BaseEventLoop = None):
        ...
        if data is None:
            raise ydlError("Couldn't find anything that matches `{}`".format(search))
        ...
        if processed_info is None:
            raise ydlError("Couldn't fetch `{}`".format(webpage_url))
        ...
        if 'entries' not in processed_info:
            ...
            while info is None:
                try:
                    ...
                    raise ydlError("Couldn't retrieve any matches for `{}`".format(webpage_url))

        return cls(ctx, discord.FFmpegPCMAudio(info['url'], **cls.FFMPEG_OPTIONS), data=info)

    @classmethod
    async def search_source(cls, ctx, search, loop: asyncio.BaseEventLoop = None):
        channel = ctx.channel
        loop = loop or asyncio.get_event_loop()
        cls.search_query = '%s%s:%s' % ('ytsearch', 10, ''.join(search))

        partial = functools.partial(cls.ydl.extract_info, cls.search_query, download=False, process=False)
        ...
        for i in info['entries']:
            ...
            lst.append(f'`{info["entries"].index(i) + 1}.` [{i.get("title")}]({v_url})\n')
        ...
        def check(msg):
            return msg.content.isdigit() == True and msg.channel == channel or str(msg.channel).lower() == 'cancel'
                ...
                if 0 < sel <= 10:
                    for key, value in info.items():
                        if key == 'entries':
                            ...
                            v_id = value[sel - 1]['id']
                            v_url = 'https://www.youtube.com/watch?v=%s' % v_id
                            partial = functools.partial(cls.ydl.extract_info, v_url, download=False)
                            data = await loop.run_in_executor(None, partial)
                    ret = cls(ctx, discord.FFmpegPCMAudio(data['url'], **cls.FFMPEG_OPTIONS), data=data)
                else:
                    ret = 'sel_invalid'
...
class Song:
    ...
    def make_embed(self):
        embed = (discord.Embed(title='Now playing', description='```css\n{0.source.title}\n```'.format(self),
                 ...
                 .set_thumbnail(url=self.source.thumb)
    ...
    async def audio_player_task(self):
        while True:
            ...
            if not self.loop:
                ...
                await self.current.source.channel.send(embed=self.current.make_embed())

            # Loop setup
            elif self.loop:
                self.now = discord.FFmpegPCMAudio(self.current.source.stream_url, **YDL.FFMPEG_OPTIONS)
...
class Music(commands.Cog):
    ...
    async def _now(self, ctx: commands.Context):
        ...
        embed = ctx.voice_state.current.make_embed()
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if voter == ctx.voice_state.current.requester:
            await ctx.send('Skipping :fast_forward:')
    ...
    async def _shuffle(self, ctx: commands.Context):
        ...
        await ctx.send('Shuffling :twisted_rightwards_arrows:')
    ...
    async def _remove(self, ctx: commands.Context, index: int):
        ...
        await ctx.send('Removed :ballot_box_with_check:')
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
        """Plays a song. If there are songs in the queue, this will be queued until the
                other songs finish playing."""
        async with ctx.typing():
            try:
                source = await YDL.create_source(ctx, search, loop=self.bot.loop)
            except ydlError as e:
    ...
    async def _search(self, ctx: commands.Context, *, search: str):
        ...
        async with ctx.typing():
            try:
                source = await YDL.search_source(ctx, search, loop=self.bot.loop)
            except ydlError as e:
                await ctx.send('An error occurred while processing this request: {}'.format(str(e)))
            else:
...
bot = commands.Bot(command_prefix='$', case_insensitive=True, description="Better Rythm Bot")
